                ...
                @unless($permissions->isEmpty())
                    @foreach ($permissions as $permission)
                        <tr>
                            ...
                            <td>
                                <i class="px-2 py-1 {{ $permission->is_active == 1 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}  text-xs rounded">
                                {{ $permission->is_active == 1 ? 'Active' : 'InActive' }}</i>
                            </td>
                            <td class="gap-5">
                                <div class="flex gap-5">
                                    ...
                                    <form method="POST" action="{{ route('admin.permissions.destroy', $permission) }}" onsubmit="return confirm('{{ trans('Are you sure to delete') }}');" style="display: inline-block;">
                                        @csrf
                                        @method('delete')
                                        <button>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    ...
                    @if ($permissions->lastPage() >1)
                        <tr><td colspan="3"> {{ $permissions->links() }} </td></tr>
                    @endif
                @else
