        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Slug
                        <span wire:click="sortBy('slug')" class="flex float-right text-sm cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75L12 3m0 0l3.75 3.75M12 3v18" class="{{ $sortBy === 'slug' && $sortDirection === 'asc' ? '' : 'text-slate-200' }}" />
                            </svg>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25L12 21m0 0l-3.75-3.75M12 21V3"  class="{{ $sortBy === 'slug' && $sortDirection === 'desc' ? '' : 'text-slate-200' }}" />
                            </svg>
                        </span>
                    </th>
                    <th>Image</th>
                ...
                @unless($categories->isEmpty())
                    @foreach ($categories as $category)
                        <tr>
                            ...
                            <td>
                                @if ($category->image)
                                    <img class=" h-20" src="{{ asset('/storage/'.$category->image) }}">
                                @endif
                            </td>
                            <td>
                                <i class="px-2 py-1 {{ $category->is_active == 1 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}  text-xs rounded">
                                {{ $category->is_active == 1 ? 'Active' : 'InActive' }}</i>
                            </td>
                            <td class="gap-5">
                                <div class="flex gap-5">
                                    ...
                                    <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" onsubmit="return confirm('{{ trans('Are you sure to delete') }}');" style="display: inline-block;">
                                        @csrf
                                        @method('delete')
                                        <button>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
